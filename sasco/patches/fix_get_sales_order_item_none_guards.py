...
# Full replacement of "Get Sales Order Item" — all None-guard fixes applied,
# stray floating line removed.
GET_SALES_ORDER_ITEM_SCRIPT = r"""############################################# RAW MATERIAL SUMMARY #############################################
tol = 0
...
def execute():
    _patch_get_sales_order_item()
    _patch_job_card_update()


def _patch_get_sales_order_item():
    name = "Get Sales Order Item"
    if not frappe.db.exists("Server Script", name):
        return
    frappe.db.set_value("Server Script", name, "script", GET_SALES_ORDER_ITEM_SCRIPT, update_modified=False)
    frappe.db.commit()


def _patch_job_card_update():
    ...
    changed = False
    for old, new in JOB_CARD_UPDATE_FIXES:
        if old in script:
